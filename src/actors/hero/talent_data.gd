	...
	match hero_subclass:
		...
		ConstantsData.HeroSubclass.CHAMPION:
			return [
				...
				_make("champion_combined_lethality", "Combined Lethality", "If the Champion attacks with a melee weapon immediately after using a different weapon's ability, that attack will execute any non-boss enemy left at below 13%/27%/40% HP.", 3, 3, hero_subclass),
			]
		ConstantsData.HeroSubclass.MONK:
			return [
				...
				_make("monk_monastic_vigor", "Monastic Vigor", "The Monk's abilities become empowered while her energy is at 100%/80%/60% or more of its cap.", 3, 3, hero_subclass),
				_make("monk_combined_energy", "Combined Energy", "Using an expensive monk ability shortly after a weapon ability (or vice versa) refunds 1 energy. Monk abilities costing 4/3/2+ energy qualify.", 3, 3, hero_subclass),
			]
	return []
