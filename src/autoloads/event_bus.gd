...
## Emitted when a hostile mob becomes aware of a hero. payload: mob (Variant)
@warning_ignore("unused_signal")
signal mob_alerted(mob: Variant)
...
## Emitted when a mob takes damage with source metadata for feedback. payload: mob_pos (int), amount (int), source (Variant)
@warning_ignore("unused_signal")
signal mob_damaged_detailed(mob_pos: int, amount: int, source: Variant)
