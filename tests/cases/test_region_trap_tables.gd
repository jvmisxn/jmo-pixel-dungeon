...
func _test_caves_uses_source_weighted_trap_table(t: Object) -> void:
	...
	var expectations: Array[Array] = [
		[0.00, "fire trap"],
		[4.0 / 31.0, "poison dart trap"],
		[8.0 / 31.0, "frost trap"],
		[12.0 / 31.0, "storm trap"],
		[16.0 / 31.0, "corrosion trap"],
		[20.0 / 31.0, "gripping trap"],
		[22.0 / 31.0, "rockfall trap"],
		[24.0 / 31.0, "guardian trap"],
		[26.0 / 31.0, "confusion gas trap"],
		[27.0 / 31.0, "summoning trap"],
		[28.0 / 31.0, "warping trap"],
		[29.0 / 31.0, "pitfall trap"],
		[30.0 / 31.0, "gateway trap"],
	]
...
func _test_city_uses_source_weighted_trap_table(t: Object) -> void:
	...
	var expectations: Array[Array] = [
		...
		[32.0 / 34.0, "pitfall trap"],
		[33.0 / 34.0, "gateway trap"],
	]
	for expectation: Array in expectations:
		var trap: Trap = level._trap_for_weighted_roll(float(expectation[0]))
		t.check(trap.trap_name == String(expectation[1]),
