...
## WandEmpower buff (the Warlock is a Mage).
...
func _test_kill_triggers_on_eat_effects(t: Object) -> void:
	...
	_with_hero(hero, func() -> void:
		...
		t.check(hero.has_buff("WandEmpower"),
			"Trigger fires on-eat talent effects (Empowering Meal WandEmpower)")
	)
	mob.free()
	hero.free()
...
func _test_roll_at_or_above_points_fails(t: Object) -> void:
	...
	_with_hero(hero, func() -> void:
		...
		t.check(not hero.has_buff("WandEmpower"), "Failed rolls grant no on-eat effects")
	)
	mob.free()
	hero.free()
